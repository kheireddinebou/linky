...
import { Copy, Edit, Trash2, QrCode, ExternalLink, BarChart3 } from 'lucide-react';
...
const URLList = ({ urls, onUrlUpdated, onUrlDeleted }: URLListProps) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await urlAPI.updateUrl(editingUrl.id.toString(), editData);
      if (response.success && response.data) {
        onUrlUpdated(response.data);
        setEditingUrl(null);
        toast.success('URL updated successfully! ✨');
      } else {
        toast.error(response.message || 'Failed to update URL');
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await urlAPI.deleteUrl(id.toString());
      if (response.success) {
        onUrlDeleted(id);
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-heading text-xl font-semibold">Your Shortened URLs</h3>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setEditData(prev => ({ ...prev, original_url: e.target.value }))}
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            >
                              {isLoading[`update-${editingUrl?.id}`] ? 'Updating...' : 'Update'}
                            </Button>
                          </div>
                        </div>
                      </DialogContent>
